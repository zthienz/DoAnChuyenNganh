...
// PayOS configuration for payment cancellation
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod === 'bank_transfer' || paymentMethod === 'payos') {
      // Đơn hàng chuyển khoản bắt đầu với trạng thái "chờ xác nhận" 
      // Sẽ được cập nhật thành "đã thanh toán" khi webhook xác nhận thành công
      // hoặc "hủy" nếu người dùng hủy tại bước quét QR
      orderStatus = 'cho_xacnhan';
      paymentStatus = 'chua_tt';
      
      console.log('🏦 Creating bank transfer order with status: cho_xacnhan (pending payment)');
    } else if (paymentMethod === 'cod') {
      // COD trực tiếp chờ xác nhận
      orderStatus = 'cho_xacnhan';
      paymentStatus = 'chua_tt';
      
      console.log('💵 Creating COD order with status: cho_xacnhan');
    } else {
      // Fallback - mọi phương thức khác đều chờ xác nhận
      orderStatus = 'cho_xacnhan';
      paymentStatus = 'chua_tt';
      
      console.log('❓ Unknown payment method, defaulting to: cho_xacnhan');
    }

    // Tạo đơn hàng
    const [orderResult] = await pool.query(
      `INSERT INTO donhang 
      (id_nguoidung, ma_donhang, trangthai, trangthai_thanhtoan, tong_tien, id_diachi, phuongthuc_thanhtoan, ghichu) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, orderCode, orderStatus, paymentStatus, totalAmount, addressId, paymentMethod, note]
    );
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    // Kiểm tra trạng thái đơn hàng - chỉ cho phép hủy khi chờ xác nhận và chưa thanh toán
    ...
    // Nếu đơn hàng chưa thanh toán và có giao dịch pending, hủy giao dịch trước
    if (order.trangthai_thanhtoan === 'chua_tt' && 
        (order.phuongthuc_thanhtoan === 'bank_transfer' || order.phuongthuc_thanhtoan === 'payos')) {
      
      // Tìm và hủy giao dịch thanh toán
      const [transactions] = await pool.query(
        'SELECT order_code FROM payment_transactions WHERE id_donhang = ? AND status = "pending"',
        [orderId]
      );
      ...
      if (transactions.length > 0) {
        try {
          ...
          await payOS.cancelPaymentLink(transactions[0].order_code, 'Cancelled by user');
          
          // Cập nhật trạng thái transaction
          await pool.query(
            'UPDATE payment_transactions SET status = "cancelled" WHERE order_code = ?',
            [transactions[0].order_code]
          );
        } catch (paymentError) {
          console.error('Error cancelling payment:', paymentError);
          // Tiếp tục hủy đơn hàng ngay cả khi không hủy được payment link
        }
      }
    }
